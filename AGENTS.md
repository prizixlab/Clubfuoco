...
- **Never hand-edit `ios-native/ClubFuoco/Core/Localization/Localizable.xcstrings`.** It is generated output: the next `gen-xcstrings.js` run rewrites the file wholesale and silently deletes any key that isn't in a source. Native-only strings go in `NATIVE_KEYS` (en/es) inside `gen-xcstrings.js`, and their ca/fr in `ios-native/scripts/translations-ca-fr.json`; everything else comes from `src/messages/{en,es}.ts`. A key that is missing renders as its own raw key on screen (`gate.accountTitle`) rather than failing the build, so this drift stays invisible until someone opens that screen — it had already eaten 16 keys by 2026-09-21. After regenerating, diff the key set against the previous catalog and confirm nothing was dropped.
