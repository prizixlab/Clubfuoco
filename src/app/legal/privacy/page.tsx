...
const SECTIONS: LegalSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'The App does not read your contacts, photos, microphone, or full calendar. Location is requested only for attendance check-in on a booking you made. By default this is foreground only — your device sends a one-off location reading when you tap "I\'m here" or open the active booking screen near arrival time. If you upgrade to "Always" in iOS Settings, the App registers a small geofence (~200 m) around each upcoming venue, active only on the night of the booking, so iOS can wake the App long enough to record your arrival automatically; we do not run any continuous background location tracking. You can revert to foreground-only or revoke location entirely at any time in Settings → Privacy → Location. If you grant write-only calendar access to add a group night, the App only creates the event you asked for — it cannot read your existing calendar.',
    ],
  },
  {
